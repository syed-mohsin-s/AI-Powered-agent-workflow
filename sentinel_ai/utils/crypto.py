...
class HashChain:
    ...
    def verify_chain(records: list[dict], algorithm: str = "sha256") -> dict:
        ...
        for i, rec in enumerate(records):
            ...
            h = hashlib.new(algorithm)
            h.update(
                json.dumps(chain_data, sort_keys=True, default=str).encode("utf-8")
            )
            computed = h.hexdigest()
            ...
            if computed != rec["record_hash"]:
                ...
                result["first_invalid_reason"] = (
                    f"Hash mismatch at record {i}: expected {rec['record_hash']}, got {computed}"
                )
                break
            ...
            if i > 0 and rec["previous_hash"] != records[i - 1]["record_hash"]:
                ...
                result["first_invalid_index"] = i
                result["first_invalid_reason"] = (
                    f"Chain break at record {i}: previous_hash doesn't match prior record"
                )
                break
