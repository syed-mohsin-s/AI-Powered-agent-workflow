        ...
        for task_id, output in shared_context.items():
            if output is None:
                ...
                if output.get("decision") == "reject":
                    ...
                    guard_ok = bool(output.get("guard_passed")) and not bool(
                        output.get("blocked")
                    )
                    checks.append(
                        {
                            ...
                            "passed": guard_ok,
                            "detail": output.get(
                                "reason", "Reliability guard check evaluated"
                            ),
                        }
                    )
                    if not guard_ok:
                        issues.append(
                            f"Reliability guard blocked execution for {task_id}"
                        )
                    ...
                    has_blocking_error = bool(exec_result.get("error")) and not bool(
                        exec_result.get("simulated")
                    )
                    ...
                    if not passed:
                        ...
                        issues.append(
                            f"Execution failure detected in {task_id}: {status}"
                        )
        ...
        checks.append(
            {
                ...
                "detail": "Compliance was "
                + ("evaluated" if compliance_checked else "NOT evaluated"),
            }
        )
