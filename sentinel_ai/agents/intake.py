...
class IntakeAgent(BaseAgent):
    ...
    async def _extract_invoice(self, data: dict) -> dict:
        ...
        return {
            "vendor_name": data.get(
                "vendor_name", self._extract_field(content, r"vendor[:\s]+(.+?)[\n]")
            ),
            "invoice_number": data.get(
                "invoice_number",
                self._extract_field(content, r"invoice\s*#?\s*[:\s]+(\S+)"),
            ),
            "date": data.get(
                "date",
                self._extract_field(
                    content, r"date[:\s]+(\d{1,2}[-/]\d{1,2}[-/]\d{2,4})"
                ),
            ),
            "total_amount": data.get(
                "total_amount",
                self._extract_number(content, r"total[:\s]+\$?([\d,]+\.?\d*)"),
            ),
            "currency": data.get("currency", "USD"),
            "po_number": data.get(
                "po_number", self._extract_field(content, r"PO\s*#?\s*[:\s]+(\S+)")
            ),
    ...
    async def _extract_meeting(self, data: dict) -> dict:
        ...
        for line in content.split("\n"):
            ...
            if any(
                kw in line_lower
                for kw in ["decided", "agreed", "approved", "will proceed"]
            ):
        ...
        for line in content.split("\n"):
            ...
            if any(
                kw in line_lower
                for kw in ["action:", "todo:", "task:", "will do", "assigned to"]
            ):
                action_items.append(
                    {"task": line.strip(), "assignee": "TBD", "deadline": "TBD"}
                )
    ...
    async def _extract_onboarding(self, data: dict) -> dict:
        ...
        return {
            ...
            "access_required": data.get(
                "access_required", ["email", "slack", "github"]
            ),
        }
