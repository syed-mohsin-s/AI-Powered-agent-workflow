...
class LLMClient:
    ...
    def is_available(self) -> bool:
        ...
        return (
            ...
            or getattr(self, "_gemini_client", None) is not None
        )
...
class BaseAgent(ABC):
    ...
    async def __call__(self, context: dict) -> TaskResult:
        ...
        try:
            # Load context-specific skills
            if self._skill_loader is None:
                from sentinel_ai.core.skills.skill_loader import \
                    get_skill_loader

                self._skill_loader = get_skill_loader()

            skills = self._skill_loader.load_skills_for_task(
                task_name=context.get("task_name", ""),
                workflow_type=context.get("workflow_type", ""),
                data=context.get("input_data", {}),
            )
            if skills:
                context["_active_skills"] = skills
                context["_skill_system_prompt"] = self._skill_loader.get_system_prompt(
                    skills
                )
                context["_skill_preferred_tools"] = (
                    self._skill_loader.get_preferred_tools(skills)
                )
            ...
            create_audit_record(
                ...
                or f"Task {'completed' if result.success else 'failed'}",
    ...
    async def llm_analyze(
        self, prompt: str, system: str = "", context: dict = None
    ) -> str:
        """Use the LLM for analysis/reasoning. Uses skill-provided system prompt when available."""
        if not system:
            # Check if context has a skill-provided system prompt
            if context and context.get("_skill_system_prompt"):
                system = context["_skill_system_prompt"]
            else:
