...
class WorkflowSubmission(BaseModel):
    ...
    workflow_type: str = Field(
        ..., description="Type: p2p, onboarding, meeting_intelligence, contract_clm"
    )
    input_data: dict = Field(
        default_factory=dict, description="Workflow-specific input data"
    )
    priority: int = Field(
...
class GoalSubmission(BaseModel):
    """Request to create and execute a goal-driven workflow."""

    goal: str = Field(..., description="Natural language description of the objective")
    constraints: Optional[dict] = Field(
        default_factory=dict, description="Constraints like time_limit, budget, etc."
    )
    priority: int = Field(
        default=5, ge=1, le=10, description="Priority 1-10 (10 = highest)"
    )
...
class WSEvent(BaseModel):
    ...
    event_type: (
        str  # workflow_update, task_update, agent_health, sla_warning, metric_update
    )
    timestamp: str
    data: dict
