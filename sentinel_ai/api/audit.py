...
async def query_audit(
    ...
    return {
        "total": len(records),
        "records": [
            r.to_spec_format() if hasattr(r, "to_spec_format") else r for r in paginated
        ],
    }
...
async def get_workflow_audit(workflow_id: str):
    ...
    records = [
        r for r in _audit_records if getattr(r, "workflow_id", "") == workflow_id
    ]
    return {
        "workflow_id": workflow_id,
        "total": len(records),
        "records": [
            r.to_spec_format() if hasattr(r, "to_spec_format") else r for r in records
        ],
    }
