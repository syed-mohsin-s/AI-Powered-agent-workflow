...
from sentinel_ai.core.dag import ExecutionGroup, WorkflowDAG
from sentinel_ai.core.event_bus import Event, EventType, get_event_bus
from sentinel_ai.core.scheduler import get_scheduler
from sentinel_ai.core.state import (InvalidTransitionError, transition_task,
                                    transition_workflow)
from sentinel_ai.core.tool_registry import (ToolCapability, ToolRegistry,
                                            get_tool_registry)
...
class WorkflowEngine:
    ...
    def register_agent(self, agent_type: str, execute_fn: Callable) -> None:
        ...
        Also creates a backward-compatible entry in the tool registry.
        """
        self._agent_registry[agent_type] = execute_fn
        # Auto-register in the tool registry for backward compatibility
        self._tool_registry.register_from_agent(
    ...
    def register_tool(self, capability: ToolCapability, execute_fn: Callable) -> None:
        """Register a tool with rich capability metadata."""
        self._tool_registry.register_tool(capability, execute_fn)
    ...
    async def _execute_workflow(self, workflow_id: str) -> None:
        ...
        try:
            ...
            # Execute groups sequentially; tasks within a group run in parallel
            while workflow.status == WorkflowStatus.RUNNING:
                execution_groups = dag.get_execution_order()

                # Find the first group that has at least one pending task
                group_to_run = None
                for group in execution_groups:
                    has_pending = any(
                        workflow.tasks.get(tid) and workflow.tasks[tid].status == TaskStatus.PENDING
                ...
                if not group_to_run:
                    # No groups have pending tasks
                    break

                await self._execute_group(workflow, group_to_run)
                ...
                if workflow.is_complete():
                    ...
                    if workflow.started_at:
                        ...
                        self._metrics.record_workflow_duration(
                            workflow.workflow_type, duration
                        )
    ...
    async def _execute_task(
        ...
        try:
            ...
            if task.result.success:
                ...
                # Dynamic DAG injection: if planner task, inject generated tasks
                if task.agent_type == "planner" and task.result.output_data.get(
                    "task_definitions"
                ):
                    await self._inject_planned_tasks(workflow, task)
                ...
                await self._handle_task_failure(
                    workflow,
                    task,
                    task.result.error_message or "Agent returned failure",
                )
    ...
    def _compute_retry_backoff(self, attempt_count: int) -> float:
        ...
        max_backoff = max(
            base, float(getattr(agents_cfg, "retry_max_backoff_seconds", 30.0))
        )
    ...
    async def _inject_planned_tasks(
        self, workflow: WorkflowExecution, planner_task: TaskDefinition
    ) -> None:
        """
        Inject tasks generated by the planner agent into the live workflow.

        Reads task definitions from the planner's output, creates
        TaskDefinition objects, adds them to the workflow's DAG, and
        rebuilds the execution plan.
        """
        task_defs_data = planner_task.result.output_data.get("task_definitions", [])
        if not task_defs_data:
            return

        dag = self._workflow_dags.get(workflow.id)
        if not dag:
            return

        injected = []
        for td in task_defs_data:
            task_def = TaskDefinition(
                id=td["id"],
                name=td["name"],
                agent_type=td.get("agent_type", "execution"),
                dependencies=td.get("dependencies", []),
                input_data=td.get("input_data", {}),
                timeout_seconds=td.get("timeout_seconds", 30),
            )
            dag.add_task(task_def)
            workflow.tasks[task_def.id] = task_def
            injected.append(task_def.id)

        # Rebuild execution plan with the new tasks
        dag._built = False
        dag.build()

        logger.info(
            f"Injected {len(injected)} planned tasks into workflow {workflow.id}",
            extra_data={"injected_tasks": injected},
        )
    ...
    async def _attempt_recovery(self, workflow: WorkflowExecution) -> bool:
        ...
        Supports semantic recovery: if the recovery agent suggests an
        alternative tool, the failed task is updated to use it.
        """
            ...
            try:
                context = {
                    ...
                    "failed_task": {
                        ...
                        "agent": task.agent_type,
                        "error": (
                            task.result.error_message if task.result else "Unknown"
                        ),
                        "attempts": task.attempt_count,
                        "tool_used": (
                            getattr(task.result, "tool_used", None)
                            if task.result
                            else None
                        ),
                    },
                ...
                if isinstance(result, TaskResult) and result.success:
                    recovery_data = result.output_data

                    # Semantic recovery: switch to alternative tool
                    if recovery_data.get("action") == "switch_tool":
                        alt_tool = recovery_data.get("alternative_tool", "")
                        if alt_tool:
                            task.input_data["tool_hint"] = alt_tool
                            logger.info(
                                f"Semantic recovery: switching task {task.id} to tool '{alt_tool}'",
                            )
